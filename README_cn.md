...
在使用 LangChain v1.0 (Alpha) 时使用此技能 - 代表从链式编排到统一代理架构根本性转变的革命性框架，搭载 LangGraph 引擎。

**功能特性：**
- 统一代理架构，使用 `create_agent` 入口点
- 从链式编排到代理开发的革命性转变
- LangGraph 引擎，支持强大的有状态、可中断工作流
- 使用 `astream_events` 的增强流式传输功能
- 解决"易于原型验证，难以投入生产"限制的生产就绪设计
- v1.0 架构的全面 API 文档

**文档结构：**
- `core_components.md` - 核心组件和架构变更
- `getting_started.md` - v1.0 概念介绍
- `other.md` - 附加文档和迁移指南
...
1. **下载所需技能的 ZIP 文件**：
   ...
   - LangChain v1.0：从仓库下载 `LangChain/langchain.zip`
   - 可以直接从 GitHub 下载，或使用：
     ```bash
     # 如果已克隆仓库
...
4. **使用技能**：
   - 打开 Claude Code
   - 直接使用技能名称：`/skill crewai` 或 `/skill langchain`
   - 或直接开始询问有关 CrewAI 或 LangChain v1.0 的问题
...
### 使用 LangChain v1.0 技能

**基本使用示例：**
```bash
# 安装后，您可以询问：
"如何在 LangChain v1.0 中使用 create_agent？"
"v1.0 中的链式架构和代理架构有什么区别？"
"展示使用 LangGraph 进行有状态工作流的示例"
"如何从基于链的开发迁移到基于代理的开发？"
"astream_events 有哪些流式传输功能？"
```

**访问文档：**
- 技能包含 `references/` 中的 v1.0 专用文档
- 询问 Claude"解释统一代理架构"或"显示 v1.0 迁移指南"
- 所有架构变更和新功能都可通过自然语言查询访问
...
| LangChain v1.0 | 1.0.0 Alpha | 2025-10-31 | ~25KB | ✅ 活跃 |
